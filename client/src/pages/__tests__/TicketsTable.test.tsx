...
function renderTable(props: Parameters<typeof TicketsTable>[0]) {
	return render(
		<MemoryRouter>
			<TicketsTable {...props} />
		</MemoryRouter>,
...
describe("TicketsTable", () => {
	describe("loading state", () => {
		it("renders the loading skeleton when isLoading is true", () => {
			renderTable({ tickets: [], isLoading: true });
			expect(document.querySelector(".animate-pulse")).toBeInTheDocument();
		});

		it("does not render any ticket data while loading", () => {
			renderTable({ tickets: mockTickets, isLoading: true });
	...
	describe("ticket data rendering", () => {
		it("renders the ticket id, subject, sender name, category label, and status", () => {
			renderTable({ tickets: mockTickets });
			// Ticket 1
	...
	describe("sender display", () => {
		it("shows senderName when it is present", () => {
			renderTable({ tickets: [mockTickets[0]] });
			expect(screen.getByText("Jane Smith")).toBeInTheDocument();
			expect(screen.queryByText("jane@example.com")).not.toBeInTheDocument();
		});

		it("falls back to senderEmail when senderName is null", () => {
			renderTable({ tickets: [mockTickets[1]] });
		...
		it("shows an em dash when both senderName and senderEmail are null", () => {
			...
			renderTable({ tickets: [ticket] });
			expect(screen.getByText("—")).toBeInTheDocument();
		});
	});

	describe("category label", () => {
		it("maps General_Question to 'General Question'", () => {
			const ticket: Ticket = {
				...mockTickets[0],
		...
		it("maps Technical_Question to 'Technical Question'", () => {
			renderTable({ tickets: [mockTickets[0]] });
			expect(screen.getByText("Technical Question")).toBeInTheDocument();
		});

		it("maps Refund_Request to 'Refund Request'", () => {
			renderTable({ tickets: [mockTickets[1]] });
			expect(screen.getByText("Refund Request")).toBeInTheDocument();
		});
